...
# ╔═╡ a9ff0e15-c35c-47f2-92f6-3c6f2a317980


# ╔═╡ f39b4739-7b63-43dc-bc2b-b65a173be1d5
d_pc = create_pcalg_gauss_dag("d_pc", df, d1_str);

# ╔═╡ 03b58c5e-6983-4bd5-812f-599885d9cbb1
d = CausalInference.kwargs_pdag_graphmakie(d_pc.g; ilabels=1:CausalInference.nv(d_pc.g), arrowsize=25, ilabels_fontsize=25)

# ╔═╡ 07476080-0687-4051-8b5b-1d6ab5e1baf9
let
	vars = names(df)
	f = Figure(; size=default_figure_resolution)
	ax = Axis(f[1, 1:3]; title="Scatter plot")
	graphplot!(d_pc.g; d...)
	ax = Axis(f[1, 4]; title="Legend")
	xlims!(0, 5)
	ylim = length(vars)
	ylims!(0, ylim + 2)
	...
	hidespines!(ax)
	annotations!("Index"; position=(1, ylim + 1))
	annotations!("Variable"; position=(3, ylim + 1))
	for i in (ylim):-1:1
		annotations!("$i"; position=(1, (ylim+i)/2))
		annotations!("$(vars[i])"; position=(3, (ylim+i)/2))
	end	
	f
end

# ╔═╡ b55eef68-fa5e-4563-b8ed-b8f2c28c6541
nv(d_pc.g)
