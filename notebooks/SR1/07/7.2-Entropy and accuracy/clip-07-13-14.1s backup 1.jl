...
let
	...
	for K in 1:6
		...
		println(size(log_lik))
		lppd_res[K, :] = lppd(log_lik)
		loo[K], loos[K], pk[K] = psisloo(log_lik)
		
		global lp = logprob(post7_2s_df, mass, df.brain_std, K)
		deviance[K] = -2sum(lppd(lp))
	end
	deviance
end

# ╔═╡ 4750c4dc-515f-46e4-aefa-a26e5a55120e
ndf = read_samples(m7_2s, :nesteddataframe)

# ╔═╡ a8b907bf-e8e6-4584-af2c-9f7eed12cd91
ndf.b

# ╔═╡ 79f3421f-fe27-4ad4-95d2-8c52b573473a
ll = hcat(ndf.b...)'

# ╔═╡ c2bd646f-b984-47e5-b586-7ba8efc3b4e1
typeof(ll)

# ╔═╡ 91c86bab-2199-4b13-8988-8ca0fdb7f614
function logprob2(ndf::DataFrame, x::Matrix, y::Vector, k=k)
    b = hcat(ndf.b...)'
	println(size(b))
...
# ╔═╡ 1a3ac925-3bd3-4913-acc8-845ac0035f83
let
	...
	for K in 1:6
		...
		if success(rc7_2s)
			#nt7_2s = read_samples(m7_2s, :namedtuple)
			ndf7_2s = read_samples(m7_2s, :nesteddataframe)
		end
		...
		lp = logprob2(ndf, mass, df.brain_std, K)
		deviance[K] = -2sum(lppd(lp))
	end
	deviance
end

# ╔═╡ d9699444-11c1-4cc7-b1da-f42f7c982791
size(log_lik)

# ╔═╡ 875b4585-2995-4ed1-ad9b-bf7d3eece48d
lp
...
manifest_format = "2.0"
project_hash = "ff1ca7fcd904eafb3611e888924ff72fb92fb2de"
